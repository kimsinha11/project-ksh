...
<%@ page import="com.KoreaIT.ksh.demo.vo.Review"%>
			...
			<tbody>
				<c:forEach var="review" items="${reviews}">
					<tr>
						...
						<th>
							<div class="badge badge-lg">${review.id}</div>
						</th>
						<th>${review.regDate.substring(0,10)}</th>
						<th class="title">
							<div class="title-text">
								<a href="detail?id=${review.id}&boardId=${review.boardId}">${review.title}</a>
							</div>
						</th>
						<th>${review.name}</th>
						<th>${review.goodReactionPoint}</th>
						<th>${review.badReactionPoint}</th>
					</tr>
				</c:forEach>
			</tbody>
		</table>
...
<div class="pagination flex justify-center mt-3">
	...
	<c:if test="${pageNum > 1}">
		<a href="?boardId=${review.boardId}&pageNum=1&${baseUri2 }">◀◀</a>
	</c:if>
	<c:if test="${pageNum > 10}">
		<a class="btn-text-link btn btn-outline btn-xs"
			href="?boardId=${review.boardId}&pageNum=${pageNum - 10}&${baseUri2 }">이전</a>
	</c:if>

	<c:forEach var="i" begin="1" end="${totalPages}" varStatus="status">
		<c:set var="baseUri"
			value="?boardId=${review.boardId}&pageNum=${i}&itemsPerPage=${itemsPerPage}&searchKeyword=${param.searchKeyword }&searchId=${param.searchId}" />
	...
	<c:if test="${pageNum < totalPages && totalPages - pageNum >= 10}">
		<a class="btn-text-link btn btn-outline btn-xs"
			href="?boardId=${review.boardId}&pageNum=${pageNum + 10}&${baseUri2 }">다음</a>
	</c:if>

	<c:if test="${pageNum < totalPages && totalPages - pageNum >= 10}">
		<a
			href="?boardId=${review.boardId}&pageNum=${totalPages}&${baseUri2 }">▶▶</a>
	</c:if>
</div>
<br />
